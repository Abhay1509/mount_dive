...
const Card = memo(({ title, price, time, image, description }) => {
  return (
    <div
      ...
      h-[468px] w-[340px] flex flex-col justify-center items-center hover:cursor-pointer"
    >
      {/* ✅ Image - Lazy loaded + object-cover for stable layout */}
      <div className="h-[200px] w-[280px] rounded-[30px] overflow-hidden bg-gray-200 mt-[30px]">
        <img
          src={image}
          alt={title}
          loading="lazy"
      ...
      <div className="p-3 text-center flex flex-col justify-center items-center">
        <h2 className="h-[21px] w-[290px] font-syne text-[18px] font-semibold text-gray-800 truncate">
          {title}
        </h2>

        <p className="font-syne font-bold text-[11px] leading-[21px] uppercase mt-2">
          {time}
        </p>
        ...
        <img
          src="/Vector.svg"
          alt=""
          className="absolute bottom-0 z-[-1] pointer-events-none select-none"
          loading="lazy"
        />

        <p className="font-syne font-medium text-[13px] leading-[21px] text-black/60 mt-4 line-clamp-3">
          {description}
        </p>
